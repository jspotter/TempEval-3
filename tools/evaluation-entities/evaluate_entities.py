...
def compute_precision_recall(gold_event, gold_timex, system_event, system_timex): 
    ...
    for eid in gold_event: 
        ...
        if debug >= 2: 
            print ' gold annotation:', gold_event[eid].annotation
   ...
   # timex precision computation 
    ...
    for tid in gold_timex:
        ...
        if debug >= 2: 
            print ' gold annotation:', gold_timex[tid].annotation
        ...
        if tid in system_timex and system_timex[tid].text.strip() != '':  
            ...
            if debug >= 1.5: 
                if g.value != s.value: 
                    print '  -> gold value:', g.annotation 
                    print '  -> system wrong value:', s.annotation 

        else: 
            if debug >= 1.5: 
                print ' gold annotation not found in system:', gold_timex[tid].annotation
    ...
    for tid in system_timex: 
        ...
        if debug >= 2: 
            print ' system annotation:', system_timex[tid].annotation                   
        if tid in gold_timex and gold_timex[tid].text.strip() != '': 
            ...
            if debug >= 1.5: 
                print ' system annotation not found in gold:', system_timex[tid].annotation
...
#    timex_performance = strict_timex_fscore*w_strict_timex_fscore + relaxed_timex_fscore*w_relaxed_timex_fscore + performance_type*w_type + performance_value*w_value
    ...
    if debug >= 0:
		...
		print 'Attribute\tClass\tTense\tAspect'
